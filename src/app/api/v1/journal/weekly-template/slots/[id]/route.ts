...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const slot = await runIdempotent(`journal:weekly-template:slot:update:${scope.teacherId}:${id}`, idempotencyKey, () =>
      updateTeacherWeeklyTemplateSlot({
        teacherId: scope.teacherId,
        slotId: id,
        actorUserId: guard.session.id,
        weekday: parsedBody.data.weekday,
        startTime: parsedBody.data.startTime ? normalizeHmTime(parsedBody.data.startTime) : undefined,
        startFrom: parsedBody.data.startFrom === undefined
          ? undefined
          : (parsedBody.data.startFrom ? normalizeIsoDate(parsedBody.data.startFrom) : null),
    ...
    return mapJournalError(error, 'Не удалось обновить слот шаблона недели');
  }
}
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await runIdempotent(`journal:weekly-template:slot:delete:${scope.teacherId}:${id}`, idempotencyKey, () =>
      deleteTeacherWeeklyTemplateSlot({
        teacherId: scope.teacherId,
        slotId: id,
        actorUserId: guard.session.id
      })
    ...
    return mapJournalError(error, 'Не удалось удалить слот шаблона недели');
  }
}
...
function mapJournalError(error: unknown, fallbackMessage: string) {
  ...
  if (message === 'WEEKLY_TEMPLATE_SLOT_NOT_FOUND') {
    return NextResponse.json({ code: 'WEEKLY_TEMPLATE_SLOT_NOT_FOUND', message: 'Слот шаблона не найден' }, { status: 404 });
  }
